...
loginRoot['height'] = 450
loginRoot['width'] = 800

loginRoot.iconbitmap('login.ico')
...
labelIP.place(x=245, y=100, width=100, height=20)

entryIP = tkinter.Entry(loginRoot, width=80, textvariable=IP1)
entryIP.place(x=365, y=100, width=130, height=20)

# 用户名标签
labelUser = tkinter.Label(loginRoot, text='昵称')
labelUser.place(x=245, y=130, width=80, height=20)

entryUser = tkinter.Entry(loginRoot, width=80, textvariable=User)
entryUser.place(x=365, y=130, width=130, height=20)
...
but.place(x=355, y=170, width=70, height=30)
            ...
            if (markk in dic) or pic[0] == '``':
                ...
                if data3 == '【群发】':
                    if data2 == user:  # 如果是自己则将则字体变为蓝色
                        ...
                        listbox.image_create(tkinter.END, image=dic[markk])# END将信息加在最后一行
